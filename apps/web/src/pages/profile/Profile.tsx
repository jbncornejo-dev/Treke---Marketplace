...
// IDs de rol
const EMPRENDEDOR_ROLE_ID = 10002;
...
export default function Profile() {
  ...
  const isAdmin = storedUser?.rol_id === ADMIN_ROLE_ID;

  // Puede crear anuncios si es emprendedor o admin
  const canCreateAds =
    !!storedUser &&
    (storedUser.rol_id === EMPRENDEDOR_ROLE_ID ||
      storedUser.rol_id === ADMIN_ROLE_ID);
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-3 justify-center md:justify-end shrink-0">
                  ...
                  {/* Emprendedor (10002) o Admin (10003) pueden crear anuncios */}
                  {canCreateAds && (
                    <Link
                      to="/admin/anuncios"
                      className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500 text-neutral-950 text-sm font-semibold hover:bg-emerald-400 transition-all shadow-sm"
      ...
      <div className="max-w-5xl mx-auto px-4 min-h-[400px]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            ...
            {!data.publicaciones.length && (
              <EmptyState
                ...
                msg="No tienes publicaciones activas."
                sub="¡Sube tu primer producto hoy!"
              />
            )}
          </div>
        )}
        ...
        {tab === "favs" && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            ...
            {(!data.favoritos || !data.favoritos.length) && (
              <EmptyState
                icon={<Heart size={48} />}
                msg="No tienes favoritos guardados."
                sub="Explora el mercado para encontrar tesoros."
              />
            )}
          </div>
        )}
                    ...
                    >
                      <td className="px-6 py-4 text-gray-500 whitespace-nowrap">
                        ...
                        <div className="text-xs text-gray-400">
                          {new Date(m.fecha_movimiento).toLocaleTimeString(
                            [],
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 max-w-[200px] sm:max-w-md truncate">
                              {m.descripcion}
                            </p>
                          </div>
                        </div>
        ...
        {tab === "impact" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <ImpactCard
              ...
              value={Number(
                data.impacto?.total_residuos_evitados ?? 0
              ).toString()}
              unit="u"
              icon="🗑️"
            />
            <ImpactCard
              label="CO₂ Evitado"
              value={Number(
                data.impacto?.total_co2_evitado ?? 0
              ).toString()}
              unit="kg"
              icon="☁️"
              color="emerald"
            ...
            <ImpactCard
              ...
              value={Number(
                data.impacto?.total_energia_ahorrada ?? 0
              ).toString()}
              unit="kWh"
              icon="⚡"
              color="amber"
            ...
            <ImpactCard
              ...
              value={Number(
                data.impacto?.total_agua_preservada ?? 0
              ).toString()}
              unit="L"
              icon="💧"
              color="blue"
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {data.reviews && data.reviews.length > 0 ? (
              ...
              <EmptyState
                ...
                msg="Aún no has recibido reseñas."
                sub="Completa intercambios para obtener calificaciones."
              />
            )}
          </div>
        )}
...
function ImpactCard({ label, value, unit, icon, color = "gray" }: any) {
  ...
  return (
    <div
      className={`p-6 rounded-2xl border ${colors[color]} flex items-center justify-between`}
    >
      <div>
